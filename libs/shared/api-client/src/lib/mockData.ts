...
type MockScalePreset = 'small' | '10k' | '100k' | '1m';
...
const SCALE_PRESET_TOTAL_RECORDS: Record<MockScalePreset, number> = {
    small: 0,
    '10k': 10_000,
    '100k': 100_000,
    '1m': 1_000_000,
...
const totalScaledRecords = SCALE_PRESET_TOTAL_RECORDS[mockScalePreset];


/**
 * Computes per-domain record counts from the selected total-scale preset.
 */
function countForDomain(baseCount: number, ratio: number): number {
    if (totalScaledRecords === 0) {
        return baseCount;
    }
    return Math.max(baseCount, Math.floor(totalScaledRecords * ratio));
}
...
function buildRisks(): RiskAssessment[] {
    const target = countForDomain(risks.length, 0.24);
...
function buildControls(): ComplianceControl[] {
    const target = countForDomain(controls.length, 0.12);
...
function buildAudits(): AuditPlan[] {
    const target = countForDomain(audits.length, 0.05);
...
function buildFindings(): AuditFinding[] {
    const target = countForDomain(findings.length, 0.03);
    ...
    return expandRecords(findings, target, (template, index) => ({
        ...
        auditPlanId: `AUD-${String((index % Math.max(1, countForDomain(audits.length, 0.05))) + 1).padStart(6, '0')}`,
        title: `${template.title} #${index + 1}`,
...
function buildPolicies(): Policy[] {
    const target = countForDomain(policies.length, 0.10);
...
function buildIncidents(): Incident[] {
    const target = countForDomain(incidents.length, 0.18);
...
function buildVendors(): Vendor[] {
    const target = countForDomain(vendors.length, 0.14);
...
function buildPartners(): Partner[] {
    const target = countForDomain(partners.length, 0.14);
...
export const mockData: MockDataset = {
    ...
    get frameworks() {
        return getDataset('frameworks', () => [...frameworks]);
    },
    get controls() {
