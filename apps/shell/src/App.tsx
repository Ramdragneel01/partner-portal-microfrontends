...
import AgenticChatPage from './components/chat/AgenticChatPage';
...
const App: React.FC = () => {
  ...
  const lastNonChatPathRef = useRef('/');

  useEffect(() => {
    const normalizedPath = location.pathname.split(/[?#]/)[0];
    if (normalizedPath.startsWith('/hub/chat')) {
      setIsAgenticChatOpen(true);
      navigate(lastNonChatPathRef.current, { replace: true });
      return;
    }

    lastNonChatPathRef.current = `${location.pathname}${location.search}${location.hash}`;
  ...
  useEventBus(AppEvent.NavigationRequested, ({ path }) => {
    ...
    if (targetPath.startsWith('/hub/chat')) {
      setIsAgenticChatOpen(true);
      return;
    }
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', bgcolor: 'background.default' }}>
      ...
      <Box sx={{ display: 'flex', flex: 1 }}>
        <SideNav
          ...
          onAgenticChatClick={() => setIsAgenticChatOpen(true)}
          agenticChatOpen={isAgenticChatOpen}
        />
        <Box
          component="main"
          id="main-content"
        ...
        >
          <Routes location={location} key={location.pathname}>
            ...
            <Route path="/hub/chat/*" element={<Navigate to="/" replace />} />
            <Route
        ...
        <ProtectedRoute moduleKey="agentic-chat">
          <AgenticChatPage
            open={isAgenticChatOpen}
