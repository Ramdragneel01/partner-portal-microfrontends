...
vi.mock('@shared/auth', () => ({
  ...
  canRoleAccessModule: (role: string, moduleKey: string) => {
    const policy: Record<string, string[]> = {
      ...
      'agentic-chat': ['admin', 'auditor', 'compliance-officer', 'partner', 'viewer'],
...
vi.mock('@mui/icons-material/SmartToy', () => ({ default: () => <span>SmartToy</span> }));
vi.mock('@mui/icons-material/FactCheck', () => ({ default: () => <span>FactCheck</span> }));
vi.mock('@mui/icons-material/BugReport', () => ({ default: () => <span>BugReport</span> }));
...
describe('SideNav', () => {
  ...
  it('renders all nav items for Admin role', () => {
    renderNav();
    expect(screen.getByRole('link', { name: /agentic chat/i })).toBeInTheDocument();
  ...
  it('invokes Agentic Chat drawer callback when provided', async () => {
    const onAgenticChatClick = vi.fn();

    render(
      <MemoryRouter initialEntries={['/risk-assessment']}>
        <SideNav open onOpenChange={vi.fn()} onAgenticChatClick={onAgenticChatClick} />
      </MemoryRouter>
    );

    await userEvent.click(screen.getByRole('link', { name: /agentic chat/i }));
    expect(onAgenticChatClick).toHaveBeenCalledTimes(1);
  });
  ...
  it('supports keyboard activation for collapse and expand control', async () => {
    renderNav();
