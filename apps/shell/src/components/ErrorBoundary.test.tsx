...
const ThrowingRemoteLoadChild: React.FC = () => {
    throw new Error('Loading chunk 31 failed.');
};
...
describe('ErrorBoundary', () => {
    ...
    it('invokes remote reload callback for remote import failures', async () => {
        const handleRemoteReload = vi.fn();

        render(
            <ErrorBoundary moduleName="Risk Assessment" onRemoteLoadRetry={handleRemoteReload}>
                <ThrowingRemoteLoadChild />
            </ErrorBoundary>
        );

        await userEvent.click(screen.getByRole('button', { name: /retry loading/i }));
        expect(handleRemoteReload).toHaveBeenCalledTimes(1);
    });
